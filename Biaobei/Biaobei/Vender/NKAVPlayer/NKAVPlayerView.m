...
@interface NKAVPlayerView()<CloseButtonDidClickDelegate>
...
- (void)settingControlUI
{
    ...
    [self.controlView.playButton addTarget:self action:@selector(play1) forControlEvents:UIControlEventTouchUpInside];
    [self.controlView.pauseButton addTarget:self action:@selector(player2) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.controlView.playerSilder.tapChangeValue = ^(float value) {
        ...
        if (weakSelf.total) {
            [weakSelf.playerItem seekToTime:CMTimeMake(self.total * value, 1.0) completionHandler:^(BOOL finished) {
...
- (void)play1{
    [self play];
    if (_delegate && [_delegate respondsToSelector:@selector(player:)]) {
        [_delegate player:1];
...
- (void)playFinished:(NSNotification *)notifi
{
    [self.playerItem seekToTime:kCMTimeZero completionHandler:^(BOOL finished) {
        if (self ->_delegate && [self ->_delegate respondsToSelector:@selector(finishPlay)] ) {
            [self ->_delegate finishPlay];
        }
    }];
    [self pause];
}
...
- (void)updateVideoSlider:(float)currentPlayTime
{
    ...
    if (self.total) {
        self.controlView.timeLabel.text = [NSString stringWithFormat:@"%@/%@", [self timeFormatted:(int)currentPlayTime], [self timeFormatted:(int) self.total]];
         self.controlView.playerSilder.trackValue = currentPlayTime / self.total;
    }else{
        self.controlView.timeLabel.text = [NSString stringWithFormat:@"%@/%@", [self timeFormatted:(int)currentPlayTime], [self timeFormatted:(int) CMTimeGetSeconds(duration)]];
...
- (NSString *)timeFormatted:(int)totalSeconds{int seconds = totalSeconds % 60;int minutes = (totalSeconds / 60) % 60;int hours = totalSeconds / 3600;
    if (hours == 0) {
...
- (void)updateVideoBufferProgress:(NSTimeInterval)buffer
{
    CMTime duration = _playerItem.asset.duration;
    if (self.total) {
        self.controlView.playerSilder.bufferValue = buffer / self.total;
...
- (void)fullScreenButtonClick {
    ...
    self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - StatusAndNaviHeight);
}
- (void)fullScreenButtonClick1{
    self.controlView.fullScreenButton.hidden = YES;
    self.controlView.shrinkScreenButton.hidden = NO;
    self.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-StatusAndNaviHeight);
}
...
- (void)orientationHandler {
    if ([UIDevice currentDevice].orientation == UIDeviceOrientationLandscapeLeft || [UIDevice currentDevice].orientation == UIDeviceOrientationLandscapeRight) {
        if (UIDeviceOrientationIsLandscape([UIDevice currentDevice].orientation)) {
            _isFullScreen = YES;
            [self fullScreenButtonClick1];
        }else {
            _isFullScreen = NO;
            [self shrinkScreenButtonClick1];
        }
